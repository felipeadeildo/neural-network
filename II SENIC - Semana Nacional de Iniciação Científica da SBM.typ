...
Foram implementadas e analisadas quatro funções de ativação principais (Sigmoid, ReLU, Tanh e Softmax), cada uma com características matemáticas distintas e aplicações específicas. A propagação direta foi codificada seguindo o princípio de transformações sequenciais entre camadas, onde a saída de cada camada torna-se a entrada da próxima, expressável como $A^(L) = sigma_L(W^(L)A^(L-1) + b^(L))$.
